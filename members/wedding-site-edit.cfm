...
<cfparam name="url.unlocked"  default="">
<cfparam name="showPremiumGate" default="false">
...
<cfif form.action EQ "save">
    ...
    <cfif isNumeric(form.siteId) && form.siteId GT 0>
        <cfquery datasource="#application.config.datasource#">
            UPDATE dbo.WeddingSites SET
                ...
                ceremony_start_time      = <cfqueryparam value="#trim(form.ceremonyStartTime)#" cfsqltype="cf_sql_varchar" null="#!len(trim(form.ceremonyStartTime))#">,
                ceremony_end_time        = <cfqueryparam value="#trim(form.ceremonyEndTime)#" cfsqltype="cf_sql_varchar" null="#!len(trim(form.ceremonyEndTime))#">,
                ...
                reception_start_time     = <cfqueryparam value="#trim(form.receptionStartTime)#" cfsqltype="cf_sql_varchar" null="#!len(trim(form.receptionStartTime))#">,
                reception_end_time       = <cfqueryparam value="#trim(form.receptionEndTime)#" cfsqltype="cf_sql_varchar" null="#!len(trim(form.receptionEndTime))#">,
    ...
    <cfelse>
        <cfquery datasource="#application.config.datasource#">
            ...
            VALUES (
                ...
                <cfqueryparam value="#trim(form.ceremonyStartTime)#" cfsqltype="cf_sql_varchar" null="#!len(trim(form.ceremonyStartTime))#">,
                <cfqueryparam value="#trim(form.ceremonyEndTime)#" cfsqltype="cf_sql_varchar" null="#!len(trim(form.ceremonyEndTime))#">,
                ...
                <cfqueryparam value="#trim(form.receptionStartTime)#" cfsqltype="cf_sql_varchar" null="#!len(trim(form.receptionStartTime))#">,
                <cfqueryparam value="#trim(form.receptionEndTime)#" cfsqltype="cf_sql_varchar" null="#!len(trim(form.receptionEndTime))#">,
    ...
    <cfcatch type="any">
        <cftry>
            ...
            <cfset notifier.notify(cfcatch, "wedding-site-edit save action", CGI.SCRIPT_NAME)>
...
<cfset editData = {
    ...
    venueName:"", venueAddress:"", ceremonyStartTime:"", ceremonyEndTime:"",
    receptionVenueName:"", receptionVenueAddress:"", receptionStartTime:"", receptionEndTime:"",
...
<cfif isNumeric(url.siteId) && url.siteId GT 0>
    ...
    <cfif editRow.recordCount>
        ...
        <!--- Premium lock check --->
        <cfif listFind(application.config.premiumTemplates, editRow.template)>
            <cfquery name="qPremiumUnlock" datasource="#application.config.datasource#">
                SELECT unlock_id FROM dbo.PremiumTemplateUnlocks
                WHERE wedding_site_id = <cfqueryparam value="#editRow.wedding_site_id#" cfsqltype="cf_sql_bigint">
                  AND template_name   = <cfqueryparam value="#editRow.template#"        cfsqltype="cf_sql_nvarchar">
            </cfquery>
            <cfif NOT qPremiumUnlock.recordCount>
                <cfset showPremiumGate = true>
            <cfelse>
                <cfset showPremiumGate = false>
            </cfif>
        <cfelse>
            <cfset showPremiumGate = false>
        </cfif>
        ...
        <cfset editData.ceremonyStartTime      = structKeyExists(editRow,"ceremony_start_time") ? editRow.ceremony_start_time : "">
        <cfset editData.ceremonyEndTime        = structKeyExists(editRow,"ceremony_end_time") ? editRow.ceremony_end_time : "">
        ...
        <cfset editData.receptionStartTime     = structKeyExists(editRow,"reception_start_time") ? editRow.reception_start_time : "">
        <cfset editData.receptionEndTime       = structKeyExists(editRow,"reception_end_time") ? editRow.reception_end_time : "">
...
.ws-textarea:focus { border-color:var(--primary); }
...
.ws-upload-zone:hover { border-color:var(--primary); color:var(--primary); }
...
.ws-add-q-btn:hover { border-color:var(--primary); color:var(--primary); }
...
.ws-submit-btn { padding:16px 48px; background:var(--primary); color:#fff; border:none; border-radius:10px; font-size:15px; font-weight:600; cursor:pointer; letter-spacing:0.03em; transition:background 0.2s; }
.ws-submit-btn:hover { background:var(--primary-hover); }
</style>
...
<div class="container" style="max-width:720px">
    ...
    <cfif url.unlocked EQ "1">
    <div class="alert alert-success" style="margin-bottom:24px">
        &#10003; Design unlocked! You can now personalize your premium wedding website.
    </div>
    </cfif>

    <!--- ============================================================
      PREMIUM GATE MODAL - shown if template is premium and not unlocked
    ============================================================ --->
    <cfif showPremiumGate>
    <cfoutput>
    <div id="premiumGate" style="position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px">
        <div style="background:#fff;border-radius:16px;padding:40px 36px;max-width:460px;width:100%;box-shadow:0 24px 64px rgba(0,0,0,0.2);text-align:center;position:relative">
            <div style="font-size:40px;margin-bottom:16px">&#11088;</div>
            <h2 style="font-size:24px;font-weight:700;font-family:var(--font-heading);color:##1a1a1a;margin-bottom:12px;line-height:1.3">Unlock This Design</h2>
            <p style="font-size:15px;color:##555;line-height:1.7;margin-bottom:8px">
                You've selected one of our premium wedding website designs.
            </p>
            <p style="font-size:15px;color:##555;line-height:1.7;margin-bottom:28px">
                Unlock this design to personalize it with your wedding details and begin editing.
            </p>
            <div style="background:##faf9f7;border:1px solid ##e5e5e5;border-radius:10px;padding:14px 20px;margin-bottom:28px;display:inline-block;text-align:center">
                <p style="font-size:13px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;color:##888;margin-bottom:4px">One-time unlock</p>
                <p style="font-size:28px;font-weight:700;color:##1a1a1a;font-family:var(--font-heading)">$14.99</p>
            </div>
            <div style="display:flex;flex-direction:column;gap:12px">
                <a href="/members/premium-checkout.cfm?siteId=#editData.siteId#&template=#URLEncodedFormat(editData.template)#"
                   style="display:block;padding:16px 24px;background:##1a1a1a;color:##fff;border-radius:10px;font-size:15px;font-weight:700;text-decoration:none;transition:background 0.2s"
                   onmouseover="this.style.background='##333'" onmouseout="this.style.background='##1a1a1a'">
                    &#11088; Unlock Design - $14.99
                </a>
                <a href="/members/wedding-sites.cfm"
                   style="display:block;padding:13px 24px;border:1px solid ##ddd;border-radius:10px;font-size:14px;font-weight:600;color:##555;text-decoration:none;background:##fff">
                    Choose a Different Design
                </a>
                <button onclick="document.getElementById('premiumGate').style.display='none'"
                        style="padding:10px 24px;background:none;border:none;font-size:13px;color:##aaa;cursor:pointer;text-decoration:underline">
                    Cancel
                </button>
            </div>
        </div>
    ...
    <div class="panel">
        ...
        <div class="ws-tabs" style="flex-wrap:wrap">
            ...
            <button type="button" class="ws-tab" onclick="switchTab('details',this)">Dress Code</button>
        ...
        <div id="tab-basics">
            ...
            <div class="ws-field">
                ...
                <input type="text" name="weddingDate" id="weddingDate" class="ws-input flatpickr-date" readonly placeholder="Select a date" value="<cfoutput>#HTMLEditFormat(editData.weddingDate)#</cfoutput>">
            </div>
            <div class="ws-field">
            ...
            <div class="ws-row">
                <div class="ws-field">
                    <label class="ws-label">Ceremony Start Time</label>
                    <input type="time" name="ceremonyStartTime" class="ws-input" value="<cfoutput>#HTMLEditFormat(editData.ceremonyStartTime)#</cfoutput>">
                </div>
                <div class="ws-field">
                    <label class="ws-label">Ceremony End Time</label>
                    <input type="time" name="ceremonyEndTime" class="ws-input" value="<cfoutput>#HTMLEditFormat(editData.ceremonyEndTime)#</cfoutput>">
                </div>
            </div>
            <div class="ws-field">
            ...
            <div class="ws-row">
                <div class="ws-field">
                    <label class="ws-label">Reception Start Time</label>
                    <input type="time" name="receptionStartTime" class="ws-input" value="<cfoutput>#HTMLEditFormat(editData.receptionStartTime)#</cfoutput>">
                </div>
                <div class="ws-field">
                    <label class="ws-label">Reception End Time</label>
                    <input type="time" name="receptionEndTime" class="ws-input" value="<cfoutput>#HTMLEditFormat(editData.receptionEndTime)#</cfoutput>">
                </div>
            </div>
            <div class="ws-field">
            ...
            <div style="display:flex;align-items:center;gap:10px;margin-top:4px">
                <input type="checkbox" name="published" id="cbPublished" <cfif editData.published>checked</cfif> style="width:16px;height:16px;accent-color:var(--primary)">
...
<script>
flatpickr(document.getElementById('weddingDate'), {
    dateFormat: 'Y-m-d',
    allowInput: false,
    disableMobile: true
});
