<!---
  royal_elegance.cfm — requires "site" struct set before cfinclude
--->
...
<title><cfoutput>#HTMLEditFormat(site.couple_name_1)# &amp; #HTMLEditFormat(site.couple_name_2)#</cfoutput> &mdash; Royal Elegance</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{background:#0D0A1A;color:#F5E6C8;font-family:Georgia,'Times New Roman',serif;line-height:1.7}
.hero{min-height:60vh;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:100px 24px 80px;background:radial-gradient(ellipse at center,#1A0A2E 0%,#0D0A1A 70%);border-bottom:1px solid rgba(201,168,76,.2);position:relative;overflow:hidden}
...
.hero-crown{font-size:2rem;margin-bottom:24px;opacity:.6;letter-spacing:.5em}
.hero p.eyebrow{color:#C9A84C;font-size:.7rem;letter-spacing:.5em;text-transform:uppercase;margin-bottom:20px}
.hero h1{font-family:Georgia,serif;font-size:clamp(3rem,9vw,6.5rem);font-weight:700;line-height:1.05;color:#F5E6C8;margin-bottom:24px;text-shadow:0 2px 20px rgba(201,168,76,.2)}
.hero h1 .amp{color:#C9A84C;font-size:.5em;letter-spacing:.3em}
.hero .crest{width:1px;height:60px;background:linear-gradient(to bottom,transparent,#C9A84C,transparent);margin:24px auto}
.hero p.date{color:#C9A84C;font-size:.8rem;letter-spacing:.3em;text-transform:uppercase}
.hero p.venue{color:rgba(245,230,200,.5);font-size:.75rem;letter-spacing:.18em;text-transform:uppercase;margin-top:8px}
nav{position:sticky;top:0;z-index:100;background:rgba(13,10,26,.97);border-bottom:1px solid rgba(201,168,76,.2);backdrop-filter:blur(8px)}
nav .inner{max-width:960px;margin:0 auto;display:flex;align-items:center;justify-content:center;overflow-x:auto}
nav a{padding:18px 20px;font-size:.72rem;letter-spacing:.15em;color:rgba(245,230,200,.6);text-decoration:none;white-space:nowrap;cursor:pointer;border-bottom:2px solid transparent;transition:color .2s;text-transform:uppercase}
nav a:hover{color:#C9A84C}
nav a.active{color:#C9A84C;border-bottom-color:#C9A84C}
.section{padding:80px 24px 60px;max-width:860px;margin:0 auto}
.section.alt{background:#110D20;max-width:100%}
...
.section-title span{color:#C9A84C;font-size:.9rem}
.section-title p{color:#C9A84C;letter-spacing:.35em;font-size:.68rem;text-transform:uppercase}
.detail{display:flex;justify-content:center;flex-wrap:wrap;gap:0;margin-bottom:48px;border:1px solid rgba(201,168,76,.2)}
.detail .col{padding:40px 48px;border-right:1px solid rgba(201,168,76,.2);text-align:center;flex:1;min-width:180px}
.detail .col:last-child{border-right:none}
.detail .label{color:#C9A84C;font-size:.65rem;letter-spacing:.35em;text-transform:uppercase;margin-bottom:12px}
.detail .val{font-family:Georgia,serif;font-size:1.5rem;font-weight:700;color:#F5E6C8}
.detail .sub{color:rgba(245,230,200,.5);font-size:.85rem;margin-top:6px}
.monogram{display:inline-flex;align-items:center;justify-content:center;width:90px;height:90px;border-radius:50%;border:1px solid #C9A84C;font-family:Georgia,serif;font-size:2rem;color:#C9A84C;margin-top:24px;box-shadow:0 0 30px rgba(201,168,76,.15)}
.story{line-height:2;font-size:1.05rem;opacity:.85;white-space:pre-wrap;max-width:680px;margin:0 auto;text-align:center}
.photos{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:4px}
.photos .photo{cursor:pointer;aspect-ratio:1;overflow:hidden}
.photos img{width:100%;height:100%;object-fit:cover;transition:transform .4s,filter .4s;filter:sepia(10%)}
.photos .photo:hover img{transform:scale(1.06);filter:sepia(0%)}
.faq-list{max-width:680px;margin:0 auto;display:flex;flex-direction:column;gap:0}
.faq-item{padding:28px 0;border-bottom:1px solid rgba(201,168,76,.15)}
.faq-item .q{font-weight:700;font-size:1rem;margin-bottom:8px;color:#C9A84C}
.faq-item .a{opacity:.7;line-height:1.8}
...
.rsvp{text-align:center;padding:100px 24px;background:radial-gradient(ellipse at center,#1A0A2E,#0D0A1A)}
.rsvp .eyebrow{color:#C9A84C;font-size:.68rem;letter-spacing:.5em;text-transform:uppercase;margin-bottom:16px}
.rsvp h2{font-family:Georgia,serif;font-size:clamp(2rem,5vw,3.5rem);font-weight:700;margin-bottom:16px;color:#F5E6C8}
.rsvp p{color:rgba(245,230,200,.55);margin-bottom:40px;max-width:440px;margin-left:auto;margin-right:auto;line-height:1.7}
.rsvp .btn{display:inline-block;padding:18px 56px;background:transparent;color:#C9A84C;font-size:.8rem;letter-spacing:.25em;text-transform:uppercase;border:1px solid #C9A84C;font-weight:600;text-decoration:none;transition:background .2s,color .2s}
.rsvp .btn:hover{background:#C9A84C;color:#0D0A1A}
footer{text-align:center;padding:60px 24px;background:#080510;border-top:1px solid rgba(201,168,76,.15)}
footer .names{font-family:Georgia,serif;font-size:1.8rem;color:#C9A84C;font-style:italic;margin-bottom:8px}
footer .date{font-size:.72rem;color:rgba(245,230,200,.3);letter-spacing:.25em;text-transform:uppercase}
footer .credit{font-size:1.1rem;font-weight:500;font-weight:500;color:rgba(245,230,200,.2);margin-top:20px}
#lightbox{position:fixed;inset:0;background:rgba(0,0,0,.95);z-index:999;display:none;align-items:center;justify-content:center;padding:20px}
#lightbox.show{display:flex}
#lightbox img{max-height:85vh;max-width:90vw;border-radius:4px;object-fit:contain}
#lightbox button{position:absolute;background:rgba(255,255,255,.1);border:none;color:#fff;border-radius:50%;cursor:pointer}
...
<div style="position:fixed;top:0;left:0;right:0;z-index:9999;background:#1a1a1a;color:#fff;display:flex;align-items:center;justify-content:space-between;padding:12px 24px;font-family:Arial,sans-serif;font-size:13px;gap:12px">
  ...
  <a href="/members/wedding-site-edit.cfm?template=royal_elegance" style="padding:8px 20px;background:#B8860B;color:#fff;border-radius:6px;font-weight:600;text-decoration:none;white-space:nowrap">Use This Template &rarr;</a>
  </cfif>
</div>
<div style="height:48px"></div>
  ...
  <p class="eyebrow">Royal Wedding Invitation</p>
...
<section class="rsvp" id="rsvp">
  ...
  <p>Please honor us with your presence on our special day.</p>
