<!---
  pearl_blossom.cfm — white flowers & pearls on champagne, blush_bouquet style
  Palette: champagne #FAF6F0, ivory #F0E8DC, sand #C8B090, taupe #7A6048, deep #3A2818
--->
...
<style>
:root{--champagne:#FAF6F0;--ivory:#F0E8DC;--ivory-dark:#DDD0C0;--sand:#C8B090;--taupe:#7A6048;--deep:#3A2818;--muted:rgba(58,40,24,.42)}
*{margin:0;padding:0;box-sizing:border-box}
html{scroll-behavior:smooth}
body{background:var(--champagne);color:var(--deep);font-family:'Jost',sans-serif;font-weight:300;line-height:1.7;overflow-x:hidden}

/* ===== NAV ===== */
nav{background:var(--champagne);border-bottom:1px solid var(--ivory-dark);text-align:center;position:relative;z-index:20}
...
nav a:hover,nav a.active{color:var(--deep);border-bottom-color:var(--sand)}

.sticky-nav{position:sticky;top:0;z-index:100;background:rgba(250,246,240,.97);border-bottom:1px solid var(--ivory-dark);display:none;text-align:center;backdrop-filter:blur(8px)}
...
.sticky-nav a:hover,.sticky-nav a.active{color:var(--deep);border-bottom-color:var(--sand)}

/* ===== HERO ===== */
.hero{position:relative;background:var(--champagne);height:70vh;min-height:520px;display:flex;align-items:center;overflow:hidden}
.hero-floral{position:absolute;top:0;right:0;height:100%;width:auto;display:block;pointer-events:none;z-index:2}
.hero-overlay{display:none}
.hero-text{position:relative;z-index:5;width:100%;text-align:center;padding:48px 56px}
...
.hero-rule .line{height:1px;width:56px;background:var(--sand);opacity:.55}
.hero-rule .heart{color:var(--sand);font-size:.9rem}
.hero-date{font-size:.72rem;letter-spacing:.3em;text-transform:uppercase;color:var(--muted);margin-bottom:6px}
.hero-venue{font-size:.7rem;letter-spacing:.18em;text-transform:uppercase;color:var(--taupe);opacity:.8}

@media(max-width:700px){
  .hero-floral{object-position:center top}
  .hero-overlay{background:rgba(250,246,240,.55)}
  .hero-text{padding:32px 24px 40px}
}
...
.pearl-rule{display:flex;align-items:center;justify-content:center;gap:14px;margin-top:16px}
.pearl-rule .line{height:1px;width:44px;background:var(--sand);opacity:.45}
.pearl-rule .dot{width:5px;height:5px;border-radius:50%;background:var(--sand);opacity:.5}

/* ===== DETAILS ===== */
.details-grid{display:flex;flex-wrap:wrap;gap:1px;background:var(--ivory-dark);max-width:860px;margin:0 auto}
.detail-block{flex:1;min-width:150px;padding:48px 32px;background:var(--champagne);text-align:center}
...
.link-btn{display:inline-block;margin-top:28px;padding:13px 36px;border:1px solid var(--sand);color:var(--taupe);text-decoration:none;font-size:.62rem;letter-spacing:.26em;text-transform:uppercase;font-family:'Jost',sans-serif;transition:background .2s,color .2s}
.link-btn:hover{background:var(--sand);color:var(--champagne)}

/* ===== RSVP ===== */
.rsvp-section{position:relative;text-align:center;padding:110px 24px;background:var(--ivory);overflow:hidden}
.rsvp-floral{position:absolute;bottom:0;right:0;height:90%;width:auto;opacity:.18;pointer-events:none}
...
.rsvp-btn{display:inline-block;padding:15px 56px;background:var(--deep);color:var(--champagne);font-size:.65rem;letter-spacing:.28em;text-transform:uppercase;text-decoration:none;font-family:'Jost',sans-serif;transition:background .25s}
.rsvp-btn:hover{background:var(--taupe)}

/* ===== FOOTER ===== */
footer{background:var(--champagne);text-align:center;padding:72px 24px;border-top:1px solid var(--ivory-dark)}
...
footer .credit{font-size:1.1rem;font-weight:500;font-weight:500;color:rgba(58,40,24,.18);margin-top:24px}

/* ===== LIGHTBOX ===== */
#lightbox{position:fixed;inset:0;background:rgba(58,40,24,.97);z-index:999;display:none;align-items:center;justify-content:center;padding:20px}
...
<div style="position:fixed;top:0;left:0;right:0;z-index:9999;background:#3A2818;color:#fff;display:flex;align-items:center;justify-content:space-between;padding:12px 24px;font-family:Arial,sans-serif;font-size:13px;gap:12px">
  ...
  <a href="/members/wedding-site-edit.cfm?template=pearl_blossom" style="padding:8px 20px;background:#C8B090;color:#3A2818;border-radius:4px;font-weight:600;text-decoration:none;white-space:nowrap">Use This Template &rarr;</a>
  </cfif>
</div>
...
<div class="hero" id="hero">
  <cfif len(heroImg)>
  <img src="<cfoutput>#HTMLEditFormat(heroImg)#</cfoutput>" alt="" class="hero-floral" onerror="this.style.display='none'">
  <cfelse>
  <img src="/assets/pearl-blossom.jpg" alt="" class="hero-floral" onerror="this.style.display='none'">
  </cfif>
...
<section class="section" id="our_story">
  <div class="section-title">
    ...
    <h2>Our Story</h2>
    <div class="pearl-rule"><div class="line"></div><div class="dot"></div><div class="line"></div></div>
  </div>
...
<section class="rsvp-section" id="rsvp">
  <cfif len(heroImg)>
  <img src="<cfoutput>#HTMLEditFormat(heroImg)#</cfoutput>" alt="" class="rsvp-floral" onerror="this.style.display='none'">
  <cfelse>
  <img src="/assets/pearl-blossom.jpg" alt="" class="rsvp-floral" onerror="this.style.display='none'">
  </cfif>
