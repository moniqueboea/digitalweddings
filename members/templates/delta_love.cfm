<!---
  delta_love.cfm — Full-bleed hero image, Delta Sigma Theta crimson & cream
  Palette: dark #1A0000, deep #2A0008, crimson #C41230, crimson-light #E8A0A8, gold #C9A84C, cream #FAF6EE
--->
...
.hero-overlay{position:absolute;inset:0;background:linear-gradient(to bottom,rgba(26,0,0,.45) 0%,rgba(26,0,0,.15) 40%,rgba(26,0,0,.82) 100%)}

/* Gold top border */
.hero-border{position:absolute;top:0;left:0;right:0;height:5px;z-index:5;background:linear-gradient(to right,var(--dark),var(--crimson),var(--crimson-light),var(--crimson),var(--dark))}
...
.hero-nav a{color:rgba(250,246,238,.75);text-decoration:none;font-size:.68rem;letter-spacing:.2em;text-transform:uppercase;font-family:'Cinzel',serif;font-weight:400;padding:10px 14px;border-bottom:1px solid transparent;transition:color .2s,border-color .2s;white-space:nowrap}
.hero-nav a:hover,.hero-nav a.active{color:var(--crimson-light);border-bottom-color:var(--crimson)}
...
.hero-dot{width:4px;height:4px;border-radius:50%;background:var(--crimson);flex-shrink:0}
...
/* Pearl rule used throughout */
.pearl-rule{width:60px;height:1px;background:linear-gradient(to right,transparent,var(--crimson-light),transparent);margin-top:16px}
...
.section-title .eyebrow{font-size:.65rem;letter-spacing:.32em;text-transform:uppercase;color:var(--crimson);display:block;margin-bottom:12px;font-family:'Cinzel',serif}
...
/* ---- DETAILS ---- */
.details-grid{display:flex;flex-wrap:wrap;gap:1px;background:rgba(196,18,48,.2);max-width:860px;margin:0 auto}
.detail-block{flex:1;min-width:150px;padding:48px 32px;background:var(--deep);text-align:center}
.detail-block .lbl{font-size:.65rem;letter-spacing:.28em;text-transform:uppercase;color:var(--crimson);margin-bottom:14px;display:block;font-family:'Cinzel',serif}
...
.g-photo:hover::after{border-color:var(--crimson)}
...
.faq-item{padding:24px 0;border-bottom:1px solid rgba(196,18,48,.15)}
...
.link-btn{display:inline-block;margin-top:24px;padding:13px 36px;border:1px solid var(--crimson);color:var(--crimson);text-decoration:none;font-size:.62rem;letter-spacing:.22em;text-transform:uppercase;font-family:'Cinzel',serif;transition:background .2s,color .2s}
.link-btn:hover{background:var(--crimson);color:var(--cream)}

/* ---- DIVIDER ---- */
.divider{width:100%;height:1px;background:linear-gradient(to right,transparent,var(--green),var(--crimson-light),var(--green),transparent)}
...
.rsvp-section::before{content:'';position:absolute;top:0;left:0;right:0;height:4px;z-index:2;background:linear-gradient(to right,var(--green),var(--crimson),var(--crimson-light),var(--crimson),var(--green))}
.rsvp-section .content{position:relative;z-index:2}
.rsvp-section .eyebrow{font-size:.68rem;letter-spacing:.28em;text-transform:uppercase;color:var(--crimson);display:block;margin-bottom:16px;font-family:'Cinzel',serif}
...
.rsvp-btn{display:inline-block;padding:16px 56px;border:1px solid var(--crimson);color:var(--crimson);font-size:.62rem;letter-spacing:.3em;text-transform:uppercase;text-decoration:none;font-family:'Cinzel',serif;transition:background .25s,color .25s}
.rsvp-btn:hover{background:var(--crimson);color:var(--cream)}

/* ---- FOOTER ---- */
footer{text-align:center;padding:70px 24px;border-top:1px solid rgba(196,18,48,.2)}
footer .names{font-family:'Great Vibes',cursive;font-size:2.6rem;color:var(--cream);margin-bottom:8px}
footer .date{font-size:.6rem;letter-spacing:.28em;text-transform:uppercase;color:var(--crimson)}
footer .ornament{color:rgba(196,18,48,.35);font-size:.9rem;margin-top:14px;letter-spacing:.4em}
...
#lightbox button{position:absolute;background:rgba(196,18,48,.12);border:none;color:var(--cream);cursor:pointer}
...
<div style="position:fixed;top:0;left:0;right:0;z-index:9999;background:#1A0000;color:#FAF6EE;display:flex;align-items:center;justify-content:space-between;padding:12px 24px;font-family:Arial,sans-serif;font-size:13px;border-bottom:1px solid rgba(196,18,48,.3);gap:12px">
  <cfif isNumeric(url.siteId) AND url.siteId GT 0>
  <span style="opacity:.75">&#128065; Previewing your wedding site</span>
  <button onclick="window.close()" style="padding:8px 20px;background:rgba(196,18,48,.15);color:#FAF6EE;border:1px solid rgba(196,18,48,.4);border-radius:4px;font-weight:600;cursor:pointer;font-size:13px">&times; Close Preview</button>
  <cfelse>
  <span style="opacity:.75">&#128065; Template preview &mdash; sample data shown</span>
  <a href="/members/wedding-site-edit.cfm?template=delta_love" style="padding:8px 20px;background:#C41230;color:#FAF6EE;border-radius:4px;font-weight:700;text-decoration:none;white-space:nowrap">Use This Template &rarr;</a>
  </cfif>
</div>
...
<div class="hero">
  ...
  <div class="hero-img">
    ...
    <cfelse>
    <img src="/assets/delta-love.jpg" alt="Delta Love" onerror="this.style.display='none'">
    </cfif>
  </div>
  ...
  <div class="hero-text">
    ...
    <img src="<cfoutput>#HTMLEditFormat(couplePhotoSrc)#</cfoutput>" alt="Couple photo"
      style="width:130px;height:130px;border-radius:50%;object-fit:cover;
             ...
             box-shadow:0 0 0 6px rgba(196,18,48,.15),0 4px 24px rgba(0,0,0,.6);
...
<cfoutput>
<cfif cdIsFuture>
<div style="position:relative;z-index:10;text-align:center;padding:18px 24px;border-bottom:1px solid rgba(196,18,48,.2);font-family:'Cinzel',serif">
  <span style="font-size:.6rem;letter-spacing:.4em;text-transform:uppercase;color:##E8A0A8">Countdown</span>
  <div style="margin:6px 0;display:flex;align-items:baseline;justify-content:center;gap:8px">
    <span style="font-size:2.4rem;font-weight:600;color:##E8A0A8;line-height:1">#cdDaysUntil#</span>
...
<div style="position:relative;z-index:10;text-align:center;padding:16px 24px;border-bottom:1px solid rgba(196,18,48,.2);font-family:'Cinzel',serif">
  <span style="font-size:.68rem;letter-spacing:.35em;text-transform:uppercase;color:##E8A0A8">Today Is The Day</span>
</div>
</cfif>
</cfoutput>
...
<!--- STICKY NAV --->
<nav id="stickyNav" style="display:none;position:sticky;top:0;z-index:40;background:var(--deep);border-bottom:1px solid rgba(196,18,48,.2);text-align:center">
...
<section class="rsvp-section" id="rsvp">
  <div class="bg">
    ...
    <cfelse>
    <img src="/assets/delta-love.jpg" alt="" onerror="this.style.display='none'">
    </cfif>
  </div>
  <div class="content">
