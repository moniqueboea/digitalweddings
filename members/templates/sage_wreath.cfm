<!---
  sage_wreath.cfm - eucalyptus wreath hero, sage + gold + white
  Palette: white #FAFAF7, sage #7A9B82, sage-light #E8EFE9, gold #B8922A, charcoal #2C3028, muted rgba(44,48,40,.42)
--->
...
<style>
:root{--white:#FAFAF7;--sage:#7A9B82;--sage-light:#E8EFE9;--sage-dark:#D0DDD1;--gold:#B8922A;--deep-gold:#8A6918;--charcoal:#2C3028;--muted:rgba(44,48,40,.42)}
...
/* ===== NAV ===== */
nav{background:var(--white);border-bottom:1px solid var(--sage-dark);text-align:center}
...
nav a:hover,nav a.active{color:var(--sage);border-bottom-color:var(--gold)}

.sticky-nav{position:sticky;top:0;z-index:100;background:rgba(250,250,247,.97);border-bottom:1px solid var(--sage-dark);display:none;text-align:center;backdrop-filter:blur(8px)}
...
.sticky-nav a:hover,.sticky-nav a.active{color:var(--sage);border-bottom-color:var(--gold)}

/* ===== HERO - wreath frame ===== */
.hero{background:var(--white);text-align:center;padding:48px 24px 64px}

/* Wreath container: image behind, text centered on top */
.wreath-wrap{position:relative;display:inline-block;width:min(680px,90vw);margin:0 auto}
.wreath-img{width:100%;display:block;height:auto}

/* Text sits centered inside the wreath opening */
.wreath-text{
  position:absolute;
  top:50%;left:50%;
...
.hero-eyebrow{font-size:.62rem;letter-spacing:.36em;text-transform:uppercase;color:var(--gold);display:block;margin-bottom:14px;font-family:'Jost',sans-serif}
.hero-couple{font-family:'Great Vibes',cursive;font-size:clamp(2rem,5vw,3.6rem);color:var(--charcoal);line-height:1.15;margin-bottom:14px}
.hero-rule{display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:12px}
.hero-rule .line{height:1px;width:28px;background:var(--gold);opacity:.5}
.hero-rule .star{color:var(--gold);font-size:.7rem}
.hero-date{font-size:.6rem;letter-spacing:.26em;text-transform:uppercase;color:var(--muted);margin-bottom:4px}
.hero-venue{font-size:.58rem;letter-spacing:.14em;text-transform:uppercase;color:rgba(122,155,130,.75)}

/* ===== SECTIONS ===== */
.section{padding:88px 56px;max-width:860px;margin:0 auto;text-align:center}
.section-sage{background:var(--sage-light);padding:88px 56px}
.section-sage .inner{max-width:860px;margin:0 auto;text-align:center}
.divider{height:1px;background:var(--sage-dark)}
...
.leaf-rule{display:flex;align-items:center;justify-content:center;gap:14px;margin-top:16px}
.leaf-rule .line{height:1px;width:44px;background:var(--sage);opacity:.35}
.leaf-rule .dot{width:5px;height:5px;border-radius:50%;background:var(--gold);opacity:.6}

/* ===== DETAILS ===== */
.details-grid{display:flex;flex-wrap:wrap;gap:1px;background:var(--sage-dark);max-width:860px;margin:0 auto}
...
/* ===== STORY ===== */
.story-body{font-family:'Cormorant Garamond',Georgia,serif;font-size:1.25rem;font-weight:300;line-height:2.1;color:var(--charcoal);white-space:pre-wrap;font-style:italic;max-width:620px;margin:0 auto}
...
.faq-item{padding:28px 0;border-bottom:1px solid var(--sage-dark)}
...
/* ===== TEXT ===== */
.text-body{font-size:.92rem;color:var(--charcoal);line-height:2;white-space:pre-wrap;max-width:600px;margin:0 auto;opacity:.75}
.link-btn{display:inline-block;margin-top:28px;padding:13px 36px;border:1px solid var(--sage);color:var(--sage);text-decoration:none;font-size:.62rem;letter-spacing:.26em;text-transform:uppercase;font-family:'Jost',sans-serif;transition:background .2s,color .2s}
.link-btn:hover{background:var(--sage);color:var(--white)}

/* ===== RSVP ===== */
.rsvp-section{text-align:center;padding:110px 24px;background:var(--sage-light);position:relative;overflow:hidden}
.rsvp-wreath{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:min(560px,80vw);opacity:.18;pointer-events:none}
...
.rsvp-btn{display:inline-block;padding:15px 56px;background:var(--sage);color:var(--white);font-size:.65rem;letter-spacing:.28em;text-transform:uppercase;text-decoration:none;font-family:'Jost',sans-serif;transition:background .25s}
.rsvp-btn:hover{background:#5E7A65}

/* ===== FOOTER ===== */
footer{background:var(--white);text-align:center;padding:72px 24px;border-top:1px solid var(--sage-dark)}
footer .names{font-family:'Great Vibes',cursive;font-size:2.8rem;color:var(--charcoal);margin-bottom:12px}
footer .date{font-size:.6rem;letter-spacing:.3em;text-transform:uppercase;color:var(--gold)}
footer .credit{font-size:1.1rem;font-weight:500;font-weight:500;color:rgba(44,48,40,.18);margin-top:24px}

/* ===== LIGHTBOX ===== */
#lightbox{position:fixed;inset:0;background:rgba(44,48,40,.96);z-index:999;display:none;align-items:center;justify-content:center;padding:20px}
...
@media(max-width:600px){
  .section,.section-sage{padding:64px 24px}
  .detail-block{padding:36px 20px}
  .wreath-text{width:46%}
  .hero-couple{font-size:1.6rem}
}
</style>
</head>
<body>
...
<div style="position:fixed;top:0;left:0;right:0;z-index:9999;background:#2C3028;color:#fff;display:flex;align-items:center;justify-content:space-between;padding:12px 24px;font-family:Arial,sans-serif;font-size:13px;gap:12px">
  ...
  <a href="/members/wedding-site-edit.cfm?template=sage_wreath" style="padding:8px 20px;background:#7A9B82;color:#fff;border-radius:4px;font-weight:600;text-decoration:none;white-space:nowrap">Use This Template &rarr;</a>
  </cfif>
</div>
...
<div style="text-align:center;padding:18px 24px;border-bottom:1px solid rgba(0,0,0,0.1);font-family:'Jost',sans-serif">
  <span style="font-size:.6rem;letter-spacing:.4em;text-transform:uppercase;color:##7A9B82">Countdown</span>
  <div style="margin:6px 0;display:flex;align-items:baseline;justify-content:center;gap:8px">
    <span style="font-size:2.4rem;font-weight:700;color:##7A9B82;line-height:1">#cdDaysUntil#</span>
...
<div style="text-align:center;padding:16px 24px;border-bottom:1px solid rgba(0,0,0,0.1);font-family:'Jost',sans-serif">
  <span style="font-size:.68rem;letter-spacing:.35em;text-transform:uppercase;color:##7A9B82">Today Is The Day</span>
</div>
</cfif>
</cfoutput>
...
<!--- HERO: wreath image with text centered inside --->
<div class="hero">
  <div class="wreath-wrap">
    <img src="/assets/sage-wreath.jpg" alt="" class="wreath-img" onerror="this.style.opacity='0'">
    <div class="wreath-text">
...
<section class="section" id="our_story">
  <div class="section-title">
    ...
    <h2>Our Story</h2>
    <div class="leaf-rule"><div class="line"></div><div class="dot"></div><div class="line"></div></div>
  </div>
  ...
  <img src="<cfoutput>#HTMLEditFormat(couplePhotoSrc)#</cfoutput>" alt="Couple photo" style="width:120px;height:120px;border-radius:50%;object-fit:cover;border:3px solid var(--sage);box-shadow:0 4px 20px rgba(0,0,0,.15);display:block;margin:36px auto 0">
</section>
</cfif>
...
<div class="divider"></div>
<div class="section-sage" id="q_and_a">
  <div class="inner">
    <div class="section-title">
...
<div class="divider"></div>
<div class="section-sage" id="travel">
  <div class="inner">
    <div class="section-title">
...
<section class="rsvp-section" id="rsvp">
  <img src="/assets/sage-wreath.jpg" alt="" class="rsvp-wreath" onerror="this.style.display='none'">
