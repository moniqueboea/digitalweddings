<!---
  blush_silk.cfm - blush chiffon fabric hero, soft & romantic
  Palette: blush #F2DDD8, rose #C49090, deep rose #7A3D42, cream #FDF8F6, warm #E8C8C4, muted rgba(90,45,48,.42)
--->
...
<style>
:root{--cream:#FDF8F6;--blush:#F2DDD8;--blush-dark:#E8C8C4;--rose:#C49090;--deep-rose:#7A3D42;--wine:#4A1F22;--muted:rgba(90,45,48,.42)}
*{margin:0;padding:0;box-sizing:border-box}
html{scroll-behavior:smooth}
body{background:var(--cream);color:var(--wine);font-family:'Jost',sans-serif;font-weight:300;line-height:1.7;overflow-x:hidden}

/* ===== HERO ===== */
.hero{position:relative;min-height:100vh;display:flex;flex-direction:column;overflow:hidden}
.hero-bg{position:absolute;inset:0;background-image:url('/assets/blush-silk.jpg');background-size:cover;background-position:center;z-index:0}
.hero-overlay{position:absolute;inset:0;background:rgba(253,248,246,.18);z-index:1}

/* NAV */
nav{position:relative;z-index:10;text-align:center;border-bottom:1px solid rgba(196,144,144,.25);background:rgba(253,248,246,.7);backdrop-filter:blur(8px)}
...
nav a:hover,nav a.active{color:var(--deep-rose);border-bottom-color:var(--rose)}

/* Sticky nav */
.sticky-nav{position:sticky;top:0;z-index:100;background:rgba(253,248,246,.97);border-bottom:1px solid var(--blush-dark);display:none;text-align:center;backdrop-filter:blur(8px)}
...
.sticky-nav a:hover,.sticky-nav a.active{color:var(--deep-rose);border-bottom-color:var(--rose)}
...
.hero-eyebrow{font-size:1rem;letter-spacing:.38em;text-transform:uppercase;color:var(--rose);margin-bottom:24px;display:block;font-family:'Jost',sans-serif}
.hero-couple{font-family:'Great Vibes',cursive;font-size:clamp(3.4rem,8vw,7rem);color:var(--wine);line-height:1.1;margin-bottom:28px;text-align:center}
.hero-rule{display:flex;align-items:center;justify-content:center;gap:16px;margin-bottom:22px}
.hero-rule .line{height:1px;width:56px;background:var(--rose);opacity:.5}
.hero-rule .star{color:var(--rose);font-size:.85rem}
.hero-date{font-size:1rem;letter-spacing:.3em;text-transform:uppercase;color:var(--muted);margin-bottom:6px}
.hero-venue{font-size:1rem;letter-spacing:.18em;text-transform:uppercase;color:rgba(196,144,144,.8)}
...
.section-title .eyebrow{font-size:1rem;letter-spacing:.32em;text-transform:uppercase;color:var(--rose);display:block;margin-bottom:12px;font-family:'Jost',sans-serif}
...
.silk-rule{display:flex;align-items:center;justify-content:center;gap:14px;margin-top:16px}
.silk-rule .line{height:1px;width:44px;background:var(--rose);opacity:.35}
.silk-rule .dot{width:5px;height:5px;border-radius:50%;background:var(--rose);opacity:.5}
...
/* ===== STORY ===== */
.story-body{font-family:'Cormorant Garamond',Georgia,serif;font-size:1.25rem;font-weight:300;line-height:2.1;color:var(--wine);white-space:pre-wrap;font-style:italic;max-width:620px;margin:0 auto}
...
/* ===== TEXT ===== */
.text-body{font-size:1rem;color:var(--wine);line-height:2;white-space:pre-wrap;max-width:600px;margin:0 auto;opacity:.75}
.link-btn{display:inline-block;margin-top:28px;padding:13px 36px;border:1px solid var(--rose);color:var(--deep-rose);text-decoration:none;font-size:.62rem;letter-spacing:.26em;text-transform:uppercase;font-family:'Jost',sans-serif;transition:background .2s,color .2s}
...
.rsvp-bg{position:absolute;inset:0;background-image:url('/assets/blush-silk.jpg');background-size:cover;background-position:center bottom;z-index:0;filter:brightness(1.04) saturate(.7)}
.rsvp-overlay{position:absolute;inset:0;background:rgba(253,248,246,.55);z-index:1}
.rsvp-content{position:relative;z-index:2;padding:100px 24px}
.rsvp-content .eyebrow{font-size:1rem;letter-spacing:.3em;text-transform:uppercase;color:var(--rose);display:block;margin-bottom:16px}
...
.rsvp-btn{display:inline-block;padding:15px 56px;background:var(--deep-rose);color:var(--cream);font-size:.65rem;letter-spacing:.28em;text-transform:uppercase;text-decoration:none;font-family:'Jost',sans-serif;transition:background .25s}
...
footer .names{font-family:'Great Vibes',cursive;font-size:2.8rem;color:var(--wine);margin-bottom:12px}
footer .date{font-size:.6rem;letter-spacing:.3em;text-transform:uppercase;color:var(--rose)}
footer .credit{font-size:1.1rem;font-weight:500;font-weight:500;color:rgba(90,45,48,.18);margin-top:24px}

/* ===== LIGHTBOX ===== */
#lightbox{position:fixed;inset:0;background:rgba(74,31,34,.97);z-index:999;display:none;align-items:center;justify-content:center;padding:20px}
...
<div style="position:fixed;top:0;left:0;right:0;z-index:9999;background:#7A3D42;color:#fff;display:flex;align-items:center;justify-content:space-between;padding:12px 24px;font-family:Arial,sans-serif;font-size:13px;gap:12px">
  ...
  <a href="/members/wedding-site-edit.cfm?template=blush_silk" style="padding:8px 20px;background:#C49090;color:#fff;border-radius:4px;font-weight:600;text-decoration:none;white-space:nowrap">Use This Template &rarr;</a>
  </cfif>
</div>
<div style="height:48px"></div>
</cfif>

<!--- HERO: full blush silk background --->
...
<div style="text-align:center;padding:18px 24px;border-bottom:1px solid rgba(0,0,0,0.1);font-family:'Jost',sans-serif">
  <span style="font-size:.6rem;letter-spacing:.4em;text-transform:uppercase;color:##C49090">Countdown</span>
  <div style="margin:6px 0;display:flex;align-items:baseline;justify-content:center;gap:8px">
    <span style="font-size:2.4rem;font-weight:700;color:##C49090;line-height:1">#cdDaysUntil#</span>
...
<div style="text-align:center;padding:16px 24px;border-bottom:1px solid rgba(0,0,0,0.1);font-family:'Jost',sans-serif">
  <span style="font-size:.68rem;letter-spacing:.35em;text-transform:uppercase;color:##C49090">Today Is The Day</span>
</div>
</cfif>
</cfoutput>
...
<section class="section" id="our_story">
  <div class="section-title">
    ...
    <h2>Our Story</h2>
    <div class="silk-rule"><div class="line"></div><div class="dot"></div><div class="line"></div></div>
  </div>
