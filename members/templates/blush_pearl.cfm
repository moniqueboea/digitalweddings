<!---
  blush_pearl.cfm - blush roses, pearls & gold rings on white satin, rose_wood structure
  Palette: warm white #FDFAF7, blush #D4956A, rose #B07080, deep #2C1A14, muted rgba(44,26,20,.55)
--->
...
<link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,400;0,600;1,300;1,400&family=Great+Vibes&family=Jost:wght@200;300;400&display=swap" rel="stylesheet">
<style>
:root{--white:#FDFAF7;--blush:#C8907A;--gold:#B8945A;--deep:#2C1A14;--muted:rgba(44,26,20,.55)}
...
.page-bg{
  ...
  inset:0;
  z-index:0;
  background-image:url('/assets/blush-pearl.jpg');
...
.page-bg::after{
  ...
  position:absolute;
  inset:0;
  background:rgba(253,250,247,.12);
}
.page-wrap{
  position:relative;
  z-index:1;
...
nav{
  ...
  border-bottom:1px solid rgba(184,148,90,.2);
  text-align:center;
...
nav a{color:rgba(44,26,20,.55);text-decoration:none;font-size:.75rem;letter-spacing:.18em;text-transform:uppercase;display:inline-block;padding:18px 16px;border-bottom:2px solid transparent;transition:color .2s,border-color .2s;white-space:nowrap;font-family:'Jost',sans-serif}
nav a:hover,nav a.active{color:var(--deep);border-bottom-color:var(--gold)}
...
.hero-eyebrow{font-size:.72rem;letter-spacing:.38em;text-transform:uppercase;color:var(--gold);margin-bottom:20px;display:block;font-family:'Jost',sans-serif}
.hero-couple{font-family:'Great Vibes',cursive;font-size:clamp(3.8rem,9vw,7.5rem);color:var(--deep);line-height:1.1;margin-bottom:32px;text-shadow:0 2px 16px rgba(255,255,255,.8)}
.hero-rule{display:flex;align-items:center;justify-content:center;gap:12px;margin-bottom:24px}
.hero-rule .line{height:1px;width:60px;background:var(--gold);opacity:.5}
.hero-rule .pearl{
  width:9px;height:9px;border-radius:50%;
  background:radial-gradient(circle at 35% 35%, #fff 0%, #ede8e4 40%, #c8b8a8 100%);
  box-shadow:0 1px 3px rgba(0,0,0,.15);
}
.hero-date{font-size:.8rem;letter-spacing:.28em;text-transform:uppercase;color:var(--muted);margin-bottom:8px}
.hero-venue{font-size:.78rem;letter-spacing:.16em;text-transform:uppercase;color:var(--gold)}

/* ---- DIVIDER ---- */
.divider{height:1px;background:rgba(184,148,90,.15);margin:0}

/* ---- SECTIONS ---- */
.section{padding:88px 56px;max-width:860px;margin:0 auto;text-align:center}
.section-dark{padding:88px 56px;background:rgba(253,250,247,.6);backdrop-filter:blur(8px)}
.section-dark .inner{max-width:860px;margin:0 auto;text-align:center}

.section-title{margin-bottom:52px}
.section-title .eyebrow{font-size:.75rem;letter-spacing:.28em;text-transform:uppercase;color:var(--gold);display:block;margin-bottom:12px;font-family:'Jost',sans-serif}
...
/* ---- DETAILS ---- */
.details-grid{display:flex;flex-wrap:wrap;gap:1px;background:rgba(184,148,90,.18);max-width:860px;margin:0 auto}
.detail-block{flex:1;min-width:150px;padding:48px 32px;background:rgba(253,250,247,.72);text-align:center;backdrop-filter:blur(6px)}
.detail-block .lbl{font-size:.72rem;letter-spacing:.28em;text-transform:uppercase;color:var(--gold);margin-bottom:14px;display:block;font-family:'Jost',sans-serif}
...
/* ---- STORY ---- */
.story-body{font-family:'Cormorant Garamond',Georgia,serif;font-size:1.25rem;font-weight:300;line-height:2.1;color:rgba(44,26,20,.8);white-space:pre-wrap;font-style:italic;max-width:620px;margin:0 auto}
...
.g-photo img{width:100%;height:100%;object-fit:cover;transition:transform .5s,filter .3s;filter:brightness(.97) saturate(.95)}
.g-photo:hover img{transform:scale(1.05);filter:brightness(1.02) saturate(1.1)}

/* ---- FAQ ---- */
.faq-list{display:flex;flex-direction:column;max-width:640px;margin:0 auto;text-align:left}
.faq-item{padding:28px 0;border-bottom:1px solid rgba(184,148,90,.15)}
...
/* ---- TEXT ---- */
.text-body{font-size:1rem;color:rgba(44,26,20,.75);line-height:2;white-space:pre-wrap;max-width:600px;margin:0 auto}
.link-btn{display:inline-block;margin-top:28px;padding:13px 36px;border:1px solid var(--gold);color:var(--gold);text-decoration:none;font-size:.72rem;letter-spacing:.22em;text-transform:uppercase;font-family:'Jost',sans-serif;transition:background .2s,color .2s}
...
.rsvp-section .bg img{width:100%;height:100%;object-fit:cover;object-position:center top;filter:brightness(.3) saturate(.7)}
.rsvp-section .content{position:relative;z-index:2}
.rsvp-section .eyebrow{font-size:.75rem;letter-spacing:.28em;text-transform:uppercase;color:rgba(184,148,90,.9);display:block;margin-bottom:16px}
.rsvp-section .script{font-family:'Great Vibes',cursive;font-size:2.6rem;color:rgba(253,250,247,.85);display:block;margin-bottom:8px}
.rsvp-section h2{font-family:'Cormorant Garamond',Georgia,serif;font-size:clamp(2.6rem,5vw,4.4rem);font-weight:300;color:#FDFAF7;margin-bottom:16px;letter-spacing:.03em;font-style:italic}
.rsvp-section p{color:rgba(253,250,247,.6);margin-bottom:44px;max-width:400px;margin-left:auto;margin-right:auto;font-size:1rem;line-height:2}
.rsvp-btn{display:inline-block;padding:15px 56px;border:1px solid rgba(253,250,247,.65);color:#FDFAF7;font-size:.72rem;letter-spacing:.28em;text-transform:uppercase;text-decoration:none;font-family:'Jost',sans-serif;transition:background .25s,color .25s}
.rsvp-btn:hover{background:#FDFAF7;color:var(--deep)}

/* ---- FOOTER ---- */
footer{text-align:center;padding:72px 24px;border-top:1px solid rgba(184,148,90,.15);background:rgba(253,250,247,.72);backdrop-filter:blur(12px)}
footer .names{font-family:'Great Vibes',cursive;font-size:2.8rem;color:var(--deep);margin-bottom:12px}
footer .date{font-size:.6rem;letter-spacing:.3em;text-transform:uppercase;color:var(--gold)}
footer .credit{font-size:1.1rem;color:rgba(44,26,20,.18);margin-top:24px}

/* ---- LIGHTBOX ---- */
#lightbox{position:fixed;inset:0;background:rgba(44,26,20,.97);z-index:999;display:none;align-items:center;justify-content:center;padding:20px}
#lightbox.show{display:flex}
#lightbox img{max-height:85vh;max-width:90vw;object-fit:contain}
#lightbox button{position:absolute;background:rgba(255,255,255,.1);border:none;color:#FDFAF7;cursor:pointer}
...
@media(max-width:600px){
  ...
  .section,.section-dark{padding:64px 24px}
  .detail-block{padding:36px 20px}
  nav a{font-size:.68rem;padding:14px 10px}
}
</style>
</head>
<body>
...
<div style="position:fixed;top:0;left:0;right:0;z-index:9999;background:#2C1A14;color:#FDFAF7;display:flex;align-items:center;justify-content:space-between;padding:12px 24px;font-family:Arial,sans-serif;font-size:13px;gap:12px;border-bottom:1px solid rgba(184,148,90,.3)">
  <cfif isNumeric(url.siteId) AND url.siteId GT 0>
  <span style="opacity:.8">&#128065; Previewing your wedding site</span>
  <button onclick="window.close()" style="padding:8px 20px;background:rgba(255,255,255,.1);color:#FDFAF7;border:none;border-radius:4px;font-weight:600;cursor:pointer;font-size:13px">&times; Close Preview</button>
  <cfelse>
  <span style="opacity:.8">&#128065; Template preview &mdash; sample data shown</span>
  <a href="/members/wedding-site-edit.cfm?template=blush_pearl" style="padding:8px 20px;background:#B8945A;color:#fff;border-radius:4px;font-weight:600;text-decoration:none;white-space:nowrap">Use This Template &rarr;</a>
  </cfif>
</div>
...
<div style="text-align:center;padding:18px 24px;border-bottom:1px solid rgba(0,0,0,0.1);font-family:'Jost',sans-serif">
  <span style="font-size:.6rem;letter-spacing:.4em;text-transform:uppercase;color:##B8945A">Countdown</span>
  <div style="margin:6px 0;display:flex;align-items:baseline;justify-content:center;gap:8px">
    <span style="font-size:2.4rem;font-weight:700;color:##B8945A;line-height:1">#cdDaysUntil#</span>
...
<div style="text-align:center;padding:16px 24px;border-bottom:1px solid rgba(0,0,0,0.1);font-family:'Jost',sans-serif">
  <span style="font-size:.68rem;letter-spacing:.35em;text-transform:uppercase;color:##B8945A">Today Is The Day</span>
</div>
</cfif>
</cfoutput>
...
<div class="hero">
  ...
  <img src="<cfoutput>#HTMLEditFormat(couplePhotoSrc)#</cfoutput>" alt="Couple photo" style="width:130px;height:130px;border-radius:50%;object-fit:cover;border:3px solid rgba(184,148,90,.4);box-shadow:0 0 0 5px rgba(184,148,90,.12),0 6px 28px rgba(44,26,20,.15);display:block;margin-bottom:22px">
...
<section class="section" id="our_story">
  <div class="section-title">
    ...
    <h2>Our Story</h2>
    <div class="gold-rule"><div class="line"></div><div class="pearl"></div><div class="pearl"></div><div class="pearl"></div><div class="line"></div></div>
  </div>
...
<section class="rsvp-section" id="rsvp">
  <div class="bg">
    ...
    <cfelse>
    <img src="/assets/blush-pearl.jpg" alt="" onerror="this.style.display='none'">
    </cfif>
  </div>
  <div class="content">
...
<button onclick="window.scrollTo({top:0,behavior:'smooth'})" id="backToTop" aria-label="Back to top" style="display:none;position:fixed;bottom:28px;right:28px;z-index:9000;width:44px;height:44px;border-radius:50%;border:none;cursor:pointer;background:rgba(184,148,90,.85);color:#fff;font-size:20px;line-height:44px;text-align:center;box-shadow:0 4px 16px rgba(184,148,90,.35);transition:opacity .2s">&uarr;</button>
