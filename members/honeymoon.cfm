    ...
    <div class="panel">
        <form method="post" action="/members/honeymoon.cfm" class="honeymoon-form">
            ...
            <div class="field">
                ...
                <input type="text" id="startDate" name="startDate" class="flatpickr-date" readonly placeholder="Select a date" value="<cfoutput>#honeymoon.recordCount && len(honeymoon.start_date) ? dateFormat(honeymoon.start_date,'yyyy-mm-dd') : ''#</cfoutput>">
            </div>
            <div class="field">
                <label for="endDate">Return Date</label>
                <input type="text" id="endDate" name="endDate" class="flatpickr-date" readonly placeholder="Select a date" value="<cfoutput>#honeymoon.recordCount && len(honeymoon.end_date) ? dateFormat(honeymoon.end_date,'yyyy-mm-dd') : ''#</cfoutput>">
            </div>
            <div class="field">
...
<script>
flatpickr(document.getElementById('startDate'), { dateFormat: 'Y-m-d', allowInput: false, disableMobile: true });
flatpickr(document.getElementById('endDate'), { dateFormat: 'Y-m-d', allowInput: false, disableMobile: true });
</script>
