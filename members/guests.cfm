...
<cfif form.action EQ "send_self">
    ...
    <cfif NOT qSelfSite.recordCount>
        <cflocation url="guests.cfm?error=selfsendfail" addToken="false">
    </cfif>
    <cfquery name="qGuestsForSelf" datasource="#application.config.datasource#">
    ...
    <cfset coordSection = "Guests & RSVP">
    ...
    <cfsavecontent variable="coordBodyHtml">
        <cfoutput>
        <cfif qGuestsForSelf.recordCount>
    ...
    <cftry>
        <cfset coordSubject = "Guest List - " & trim(qSelfSite.couple_name_1) & " & " & trim(qSelfSite.couple_name_2)>
        ...
        <cflocation url="guests.cfm?selftest=1" addToken="false">
    <cfcatch>
        <cflocation url="guests.cfm?error=selfsendfail" addToken="false">
    </cfcatch>
    </cftry>
...
<cfif form.action EQ "send_coordinator">
    ...
    <cfif NOT qCoordSite.recordCount OR NOT len(trim(qCoordSite.coord_email))>
        <cflocation url="guests.cfm?error=noemail" addToken="false">
    </cfif>
    <cfquery name="qGuestsForCoord" datasource="#application.config.datasource#">
        SELECT name, email, rsvp_status, plus_one, plus_one_name, dietary_restrictions, guest_group
        FROM dbo.Guests
        WHERE user_id = <cfqueryparam value="#userId#" cfsqltype="cf_sql_bigint">
        ORDER BY name
    ...
    <cfsavecontent variable="coordBodyHtml">
        <cfoutput>
        <cfif qGuestsForCoord.recordCount>
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse">
            <tr style="background:##e8f0e9">
                <th style="padding:8px 10px;text-align:left;font-size:12px;color:##3a5e3e;font-family:Arial,sans-serif;border-bottom:1px solid ##b2cbb5">Name</th>
                <th style="padding:8px 10px;text-align:left;font-size:12px;color:##3a5e3e;font-family:Arial,sans-serif;border-bottom:1px solid ##b2cbb5">RSVP</th>
    ...
    <cftry>
        <cfset coordSubject = "Guest List - " & trim(qCoordSite.couple_name_1) & " & " & trim(qCoordSite.couple_name_2)>
        <cfmail to="#trim(qCoordSite.coord_email)#"
                ...
                subject="#coordSubject#"
                type="html" timeout="60"><cfinclude template="email-coordinator-body.cfm"></cfmail>
        <cflocation url="guests.cfm?coordsent=1" addToken="false">
    <cfcatch>
        <cflocation url="guests.cfm?error=sendfail" addToken="false">
    </cfcatch>
    </cftry>
</cfif>
...
<div class="container">
    <div class="page-header" style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:16px">
        ...
        <div style="display:flex;gap:10px;margin-top:8px;flex-wrap:wrap">
            <a href="/members/edit-invite.cfm" class="btn btn-secondary">Edit Invite Email</a>
            <form method="post" action="/members/guests.cfm" style="margin:0">
            ...
            <form method="post" action="/members/guests.cfm" style="margin:0">
                ...
                <button type="submit" class="btn btn-ghost">&#128140; Send to Coordinator</button>
            </form>
        </div>
    </div>
    ...
    <div class="alert alert-success" style="margin-bottom:24px">Guest list sent to your coordinator!</div>
    </cfif>
    <cfif url.selftest EQ "1">
    <div class="alert alert-success" style="margin-bottom:24px">Guest list preview sent to <cfoutput>#HTMLEditFormat(session.user.email)#</cfoutput> - check your inbox!</div>
    </cfif>
    ...
    <cfif url.sent EQ "1">
        <cfif len(url.mailerror)>
            <div class="alert alert-success" style="margin-bottom:24px">Guest added, but the invitation email could not be sent. The administrator has been notified.</div>
        <cfelse>
    ...
    <cfif url.resent EQ "1">
        <cfif len(url.mailerror)>
            <div class="alert alert-error" style="margin-bottom:24px">Could not resend invitation email. The administrator has been notified.</div>
        <cfelse>
