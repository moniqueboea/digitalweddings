...
<head>
    ...
    <style>
        :root { color-scheme: dark; --gold: #c6a15b; --panel: #171717; --line: #343434; }
        * { box-sizing: border-box; }
        body { margin: 0; background: #0d0d0d; color: #f5f1e8; font: 15px/1.5 Arial, sans-serif; }
        ...
        input, textarea { width: 100%; border: 1px solid #4a4a4a; border-radius: 8px; background: #090909; color: white; padding: 12px; }
        textarea { min-height: 300px; resize: vertical; font: 14px/1.45 ui-monospace, SFMono-Regular, Menlo, monospace; }
        button, .btn-tool { display: inline-block; border: 0; border-radius: 8px; background: var(--gold); color: #16120a; padding: 11px 18px; font-weight: 700; cursor: pointer; text-decoration: none; }
        .secondary { background: #333; color: white; }
        .danger  { color: #ffb4b4; background: #3c1717; border: 1px solid #6d2929; padding: 12px; border-radius: 8px; }
        .success { color: #c8f7ce; background: #13351a; border: 1px solid #286637; padding: 12px; border-radius: 8px; }
        .actions { display: flex; gap: 10px; flex-wrap: wrap; margin-top: 14px; }
        .topbar  { display: flex; align-items: center; justify-content: space-between; gap: 16px; }
        .muted   { color: #b6b6b6; }
        .table-wrap { overflow: auto; max-height: 580px; border: 1px solid var(--line); border-radius: 8px; }
        table { border-collapse: collapse; width: 100%; background: #101010; }
        th, td { border-bottom: 1px solid #303030; padding: 9px 11px; text-align: left; white-space: nowrap; vertical-align: top; }
        th { position: sticky; top: 0; background: #28231a; color: #f4d99d; }
        code { color: #f4d99d; }
    </style>
</head>
<body>
<main>
    ...
    <div class="topbar">
        <div>
            <h1>Digital Weddings Database Tool</h1>
            <p class="muted">Datasource: <code><cfoutput>#HTMLEditFormat(application.config.datasource)#</cfoutput></code> &mdash; <a href="/admin/index.cfm" style="color:var(--gold)">Admin Home</a></p>
        </div>
    </div>
    ...
    <section class="card">
        ...
        <form method="post" action="/admin/db.cfm">
        ...
        <form method="post" action="/admin/db.cfm" style="margin-top:10px">
            <input type="hidden" name="csrf_token" value="<cfoutput>#HTMLEditFormat(session.databaseToolCsrf)#</cfoutput>">
